 ...
 */
package org.dcs.commons.apache.avro.io;
...
// FIXME: This class overrides the JsonDecoder class and
//        should be removed once https://issues.apache.org/jira/browse/AVRO-1582
//        is fixed. Refer to comment above method : isNullableSingle
 ...
 * */
public class StdJsonDecoder extends ParsingDecoder
  ...
  private StdJsonDecoder(Symbol root, InputStream in) throws IOException {
    super(root);
    configure(in);
  }

  private StdJsonDecoder(Symbol root, String in) throws IOException {
    super(root);
    configure(in);
  }

  public StdJsonDecoder(Schema schema, InputStream in) throws IOException {
    this(getSymbol(schema), in);
  }

  public StdJsonDecoder(Schema schema, String in) throws IOException {
    this(getSymbol(schema), in);
  }
   ...
   */
  public StdJsonDecoder configure(InputStream in) throws IOException {
    if (null == in) {
   ...
   * @return this JsonDecoder
   */
  public StdJsonDecoder configure(String in) throws IOException {
    if (null == in) {
  ...
  /**
   * The isNullableSingle, getNullableSingle and updated readIndex methods
   * have been taken from
   * https://github.com/zolyfarkas/avro/blob/trunk/lang/java/avro/src/main/java/org/apache/avro/io/ExtendedJsonDecoder.java
   * to solve the problem of decoding union types with two types of which one is of 'null' type,
   * e.g. {"name": "institutionCode",  "type": ["null", "string"], "default": null}
   * Decoding this with the current JsonDecoder gives the error,
